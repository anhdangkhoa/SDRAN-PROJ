                                        ...
                                        bool                              is_nr)
{
  for (size_t i = 0; i < num_ue; i++) {
    ...
    uint64_t timeSinceEpoch = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    fmt::print("{:>3.5}", (is_nr) ? "nr" : "lte");
    fmt::print(" {:>4}", mac.ues[i].pci);
    fmt::print("{:>5x}", mac.ues[i].rnti);
    file << std::to_string(timeSinceEpoch) <<";";
    string strisnr;
    if (is_nr) {strisnr = "nr";} else {strisnr = "lte";};
    file << strisnr <<";";
    file << std::to_string(mac.ues[i].pci) <<";";
    file << std::to_string(mac.ues[i].rnti) <<";";

    if (not iszero(mac.ues[i].dl_cqi)) {
      fmt::print("  {:>3}", int(mac.ues[i].dl_cqi));
      file << std::to_string(int(mac.ues[i].dl_cqi)) <<";";
    } else {
      fmt::print("  {:>3.3}", "n/a");
      file << "n/a" <<";";
    }
    fmt::print("   {:>1}", int(mac.ues[i].dl_ri));
    file << std::to_string(int(mac.ues[i].dl_ri)) <<";";
    float dl_mcs = (is_nr) ? mac.ues[i].dl_mcs : phy[i].dl.mcs;
    if (not isnan(dl_mcs)) {
      fmt::print("   {:>2}", int(dl_mcs));
      file << std::to_string(int(dl_mcs)) <<";";
    } else {
      fmt::print("   {:>2}", 0);
    ...
    if (mac.ues[i].tx_brate > 0) {
      ...
      file << float_to_eng_string((float)mac.ues[i].tx_brate / (mac.ues[i].nof_tti * 1e-3), 1) <<";";
    } else {
      fmt::print(" {:>6}", 0);
      file << std::to_string(0) <<";";
    }
    ...
    file << std::to_string(mac.ues[i].tx_pkts - mac.ues[i].tx_errors) <<";";
    file << std::to_string(mac.ues[i].tx_errors) <<";";
    if (mac.ues[i].tx_pkts > 0 && mac.ues[i].tx_errors) {
      fmt::print(" {:>3}%", int((float)100 * mac.ues[i].tx_errors / mac.ues[i].tx_pkts));
      file << std::to_string(int((float)100 * mac.ues[i].tx_errors / mac.ues[i].tx_pkts));
    } else {
      fmt::print(" {:>3}%", 0);
      file << std::to_string(0);
    }
...
void metrics_stdout::set_metrics(const enb_metrics_t& metrics, const uint32_t period_usec)
{
  if (!do_print || enb == nullptr) {
    ...
    if (!file.is_open()){
      file.open("/tmp/enb_metrics.csv", std::ios_base::out);
    }
  }
